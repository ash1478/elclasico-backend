...
module.exports.getSlots = async (req, res) => {
  ...
  if (!venueId)
    return res.status(404).send(failureResponseMapper("Venue Id not provided"));
  ...
  try {
    ...
    if (!venue)
      return res
        .status(404)
        .send(failureResponseMapper("Venue with given Id not found"));
    ...
    if (cacheClient.has(venueSlotsKey)) {
      if (weekDayCode === "*")
        return res
          .status(200)
          .send(
            successResponseMapper(JSON.parse(cacheClient.get(venueSlotsKey)))
          );
      return res
        .status(200)
        .send(
          successResponseMapper(
            JSON.parse(cacheClient.get(venueSlotsKey)).filter(
              (slots) => slots.weekDayCode === Number(weekDayCode)
            )
          )
        );
    }
    ...
    for (let i = 1; i < 8; i++) {
      ...
      if (bookedDataPerDay) {
        bookedDataPerDay.slots.forEach((slot) => {
          bookedSlotsPerDay.push(
            ...getSlotTimings(slot.startTime, slot.endTime)
          );
        });
      }
      bookedData.push({
    ...
    for (let i = 0; i < totalAvailableSlots.length; i++) {
      totalAvailableSlots[i].slots = totalAvailableSlots[i].slots.filter(
        (slot) => {
    ...
    if (weekDayCode === "*")
      return res.status(200).send(successResponseMapper(totalAvailableSlots));
    return res
      .status(200)
      .send(
        successResponseMapper(
          totalAvailableSlots.filter(
            (slots) => slots.weekDayCode === Number(weekDayCode)
          )
        )
      );
